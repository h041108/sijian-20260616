// ─── 官方场景模板库 ─────────────────────────────
// 30个预设场景参数模板 + 稳定预览图（内联SVG，零外部依赖）
// 选中后自动填入影片工厂的环境/光线/色调等参数

export interface SceneTemplate {
  id: string; name: string; icon: string; category: string; previewUrl?: string
...
// 根据模板名称生成确定性渐变占位图（base64内联SVG，无网络请求）
const COLORS = ["#F59E0B","#EC4899","#14B8A6","#3B82F6","#8B5CF6","#EF4444","#22C55E","#06B6D4"]
function previewUrl(name: string): string {
  const chars = name.replace(/[^一-龥]/g, "").slice(0, 2) || name.slice(0, 2)
  const idx = name.length % COLORS.length
  const c1 = COLORS[idx]; const c2 = COLORS[(idx + 1) % COLORS.length]
  const svg = `<svg xmlns='http://www.w3.org/2000/svg' width='400' height='225'>
    <defs><linearGradient id='g'><stop offset='0%' stop-color='${c1}33'/><stop offset='100%' stop-color='${c2}33'/></linearGradient></defs>
    <rect width='400' height='225' fill='url(#g)' rx='8'/>
    <text x='200' y='118' text-anchor='middle' fill='white' font-size='48' font-weight='bold' font-family='system-ui' opacity='0.85'>${chars}</text>
    <text x='200' y='148' text-anchor='middle' fill='white' font-size='11' font-family='system-ui' opacity='0.25'>${name}</text>
  </svg>`
  return "data:image/svg+xml;base64," + btoa(svg)
}

export const SCENE_TEMPLATES: SceneTemplate[] = [
  { id:"st_01",name:"赛博朋克夜城",icon:"🌃",category:"城市",previewUrl:previewUrl("st_01"),
