  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-6 space-y-8">
      <div className="text-center space-y-2">
        <h1 className="text-2xl font-extrabold text-gray-900">Agent Center</h1>
        <p className="text-sm text-gray-400">15 agents for media creation</p>
      </div>
      {GROUP_ORDER.map(gk => {
        const g = groups?.[gk]
        ...
        return (
          <div key={gk} className={"border-l-4 rounded-xl p-4 " + (COLORS[gk] || "")}>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
              {g.agents.map((a: any) => a.hasStandaloneUI ? (
                ...
                <div key={a.id} className="flex items-center gap-2.5 px-3 py-2.5 rounded-xl bg-white/80 border border-gray-200/60 opacity-60">
                  <span className="text-lg">{a.icon}</span>
                  <div>
