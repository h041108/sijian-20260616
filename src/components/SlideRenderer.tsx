...
export default function SlideRenderer({ title, content, onRecordingComplete, width = 1080, height = 1920 }: SlideRendererProps) {
  ...
  useEffect(() => {
    if (content) setDeck(parseContentToSlides(content, title))
  }, [content, title])
  ...
  const renderSlide = useCallback((slideIdx: number, slideDeck: SlideDeckConfig) => {
    ...
    grad.addColorStop(0, `${slideDeck.accentColor}15`)
    grad.addColorStop(1, `${slideDeck.accentColor}05`)
    ctx.fillStyle = grad
    ctx.fillRect(0, 0, w, h)
    ...
    if (slide.type === "title") {
      ...
      ctx.strokeStyle = `${slideDeck.accentColor}30`
      ctx.lineWidth = 1
      ctx.beginPath()
